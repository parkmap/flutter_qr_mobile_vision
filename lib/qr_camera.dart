...
final WidgetBuilder _defaultNotStartedBuilder = (context) => Center(
      child: Column(
        children: [CircularProgressIndicator(), Text("Camera Loading ...")],
      ),
    );
final WidgetBuilder _defaultOffscreenBuilder = (context) => Center(
      child: Column(
        children: [Text("Camera Paused.")],
      ),
    );
...
final ErrorCallback _defaultOnError = (BuildContext context, Object error) {
  ...
  return Center(
    child: Column(
      children: [Text("Error reading from camera...")],
    ),
  );
};
